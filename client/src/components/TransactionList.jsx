...
// --- MAIN LIST COMPONENT ---
const TransactionList = ({ newTransactions }) => {
  ...
  const fetchTransactions = useCallback(async (pageNum, filterDates) => {
    ...
    try {
      ...
      if (filterDates?.from && filterDates?.to) query += `&from=${filterDates.from}&to=${filterDates.to}`;
  ...
  const handleFilter = () => {
    if (!fromDate || !toDate) { toast.warn("Please select both From and To dates"); return; }
    setPage(1);
    fetchTransactions(1, { from: fromDate, to: toDate });
  };

  const handleReset = () => { setFromDate(""); setToDate(""); setPage(1); fetchTransactions(1, {}); };

  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this transaction?")) return;
    try { await API.delete(`/transactions/${id}`); toast.success("Transaction deleted successfully!"); fetchTransactions(page, { from: fromDate, to: toDate }); }
    catch (err) { toast.error(err.response?.data?.error || "Failed to delete transaction"); }
  };

  const handleUpdate = async (updatedData) => {
    if (!editingTransaction) return;
    try { await API.put(`/transactions/${editingTransaction._id}`, updatedData); toast.success("Transaction updated successfully!"); setIsModalOpen(false); setEditingTransaction(null); fetchTransactions(page, { from: fromDate, to: toDate }); }
    catch (err) { toast.error(err.response?.data?.error || "Failed to update transaction"); }
  };

  const handleOpenModal = (transaction) => { setEditingTransaction(transaction); setIsModalOpen(true); };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-5xl">
      ...
      <AnimatePresence>
        {isModalOpen && editingTransaction && (
          <EditTransactionModal transaction={editingTransaction} onClose={()=>setIsModalOpen(false)} onSave={handleUpdate} />
        )}
      </AnimatePresence>
    </motion.div>
  );
